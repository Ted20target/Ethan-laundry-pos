...
function updateUserDisplay() {
  if (!state.user) {
    ...
    archiveMonthBtn.classList.add("hidden");
    return;
  }

  userMenuBtn.textContent = `${state.user.username} (${state.user.role})`;
  archiveMonthBtn.classList.toggle("hidden", state.user.role !== "Administrator");
}
    ...
    <tr>
      ...
      <td>
        ${getNextStatus(order.status)
          ? `<button class="ghost-btn" data-update-order-status="${order.id}" data-next-status="${getNextStatus(order.status)}">Mark ${getNextStatus(order.status)}</button>`
...
archiveMonthBtn.addEventListener("click", async () => {
  const confirmed = window.confirm(
    "Archive all current orders and payments from the active dashboard? This will keep them in the database history but remove them from the live dashboard."
  );
  ...
  try {
    const result = await apiFetch("/api/orders/archive", { method: "POST" });
    await loadDashboardData();
    renderAll(globalSearch.value);
...
document.addEventListener("click", async (event) => {
  ...
  const updateOrderStatusId = event.target.dataset.updateOrderStatus;
  const nextStatus = event.target.dataset.nextStatus;

  try {
    if (updateOrderStatusId && nextStatus) {
      await apiFetch(`/api/orders/${updateOrderStatusId}/status`, {
        method: "PATCH",
        body: JSON.stringify({ status: nextStatus })
      });
      await loadDashboardData();
      renderAll(globalSearch.value);
      showMessage(`Order marked ${nextStatus}.`);
      return;
    }
